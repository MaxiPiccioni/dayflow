...
const fromApiEvent = (event) => ({ id: event.id, title: event.title, date: event.event_date, time: event.time, type: event.type, color: event.color, done: event.done, important: event.important });
const toApiEvent = (event) => ({ title: event.title, event_date: event.date, time: event.time, type: event.type || null, color: event.color, important: event.important || false, ...(event.repeat_days ? { repeat_days: event.repeat_days } : {}) });
...
function EventModal({ event, date, categories, save, remove, close }) {
  const [form, setForm] = useState(event || { title: "", date, time: "09:00", type: "", color: "#d9f99d", done: false, important: false });
      ...
      <form onSubmit={submit}>
        <div className="mt-5 space-y-3">
          ...
          <div className="flex items-center justify-between rounded-xl border border-zinc-200 p-3 dark:border-zinc-700">
            <span className="text-sm font-medium">Evento importante</span>
            <Switch checked={form.important || false} onChange={(value) => update("important", value)} label="Evento importante" />
          </div>
...
function Home({ user, logout, dark, setDark, wave, setWave }) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- syncs initial modal/tab from the ?open= deep link (Action Button shortcut), window.location isn't available during render
    if (open === "tarea") { setActive("calendario"); setDate(today); setTaskModal({ date: today }); }
    else if (open === "evento") { setActive("calendario"); setDate(today); setEventModal({ date: today }); }
  ...
  const progressTotal = todayTasks.length + todayEvents.length + habits.length; const progressDone = todayTasks.filter((task) => task.done).length + todayEvents.filter((event) => event.done).length + habits.filter((habit) => habit.progress >= 100).length;
  const upcomingImportantEvent = events.filter((event) => event.important && event.date >= todayStr).sort((a, b) => a.date.localeCompare(b.date))[0];
  const countdownText = (() => {
    if (!upcomingImportantEvent) return null;
    const daysLeft = daysUntil(upcomingImportantEvent.date);
    if (daysLeft === 0) return `Hoy es ${upcomingImportantEvent.title}`.toUpperCase();
    if (daysLeft === 1) return `Falta un día para ${upcomingImportantEvent.title}`.toUpperCase();
    return `Faltan ${daysLeft} días para ${upcomingImportantEvent.title}`;
  })();
