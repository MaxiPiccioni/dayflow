...
function PeriodDetailModal({ period, entries, close }) {
  ...
  [...entries].sort((a, b) => a.date.localeCompare(b.date)).forEach((entry) => { const entryValue = value(entry); dateColors[entry.date] = entryValue > 0 && remainingPaid >= entryValue ? "#bbf7d0" : "#fef08a"; remainingPaid -= entryValue; });
...
export function HoursPanel({ period, setPeriod, payments, setPayments, entries, setEntries, closedPeriods, setClosedPeriods, onSaveEntry, onRemoveEntry }) {
  ...
  entries.forEach((entry) => { if (!(entry.date in dateColors)) dateColors[entry.date] = "#e4e4e7"; });
