...
function PrintHoursList({ rows }) {
  return <div className="hours-print-list hidden mt-6">
    <strong className="mb-2 block text-sm">Detalle de jornadas</strong>
    <table className="w-full border-collapse text-xs">
      <thead><tr className="border-b border-zinc-400"><th className="py-1 text-left font-semibold">Fecha</th><th className="py-1 text-left font-semibold">Horas</th><th className="py-1 text-left font-semibold">Feriado</th><th className="py-1 text-left font-semibold">Horas extra</th></tr></thead>
      <tbody>
        {rows.map((row) => <tr key={row.date} className="border-b border-zinc-200">
          <td className="py-1">{new Date(`${row.date}T12:00:00`).toLocaleDateString("es-AR")}</td>
          <td className="py-1">{row.hours.toFixed(2)}</td>
          <td className="py-1">{row.holiday ? <span style={{ color: "#000", fontWeight: 700 }}>✓</span> : ""}</td>
          <td className="py-1">{(row.extra ? row.hours : 0).toFixed(2)}</td>
        </tr>)}
      </tbody>
    </table>
  </div>;
...
function HoursEntryModal({ entry, date, onSave, onRemove, onClose }) {
  ...
  return <Modal title={entry ? "Editar jornada" : "Nueva jornada"} close={onClose}><form onSubmit={submit}><div className="mt-5 grid grid-cols-2 gap-3"><label className="text-xs text-zinc-500">Fecha<input type="date" value={form.date} onChange={(event) => update("date", event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs text-zinc-500">Horas manuales<input type="number" min="0" step="0.25" value={form.hours} onChange={(event) => update("hours", event.target.value)} placeholder="Opcional" className={`${inputClass} mt-1`} /></label><label className="text-xs text-zinc-500">Desde<input type="time" value={form.from} onChange={(event) => update("from", event.target.value)} className={`${inputClass} mt-1`} /></label><label className="text-xs text-zinc-500">Hasta<input type="time" value={form.to} onChange={(event) => update("to", event.target.value)} className={`${inputClass} mt-1`} /></label><label className="flex items-center gap-2 text-xs text-zinc-500"><input type="checkbox" checked={form.extra} onChange={(event) => update("extra", event.target.checked)} /> Horas extra</label><label className="flex items-center gap-2 text-xs text-zinc-500"><input type="checkbox" checked={form.holiday} onChange={(event) => update("holiday", event.target.checked)} /> Feriado</label></div><div className="mt-6 flex gap-2">{entry && <button type="button" onClick={() => onRemove(entry.id)} className="rounded-xl border border-red-200 px-4 py-3 text-sm text-red-600">Eliminar</button>}<button type="submit" className="flex-1 rounded-xl bg-zinc-900 py-3 text-sm font-semibold text-white">Guardar jornada</button></div></form></Modal>;
}
...
function Saldos({ payments, addPayment, removePayment, readOnly }) { const [amount, setAmount] = useState(""); const [method, setMethod] = useState("Transferencia"); const [date, setDate] = useState(new Date().toISOString().slice(0, 10)); const submit = (event) => { event.preventDefault(); if (Number(amount) <= 0) return; addPayment({ amount: Number(amount), method, date }); setAmount(""); }; return <Card className="hours-saldos"><CardHeader eyebrow="Pagos recibidos" title="Saldos" />{readOnly ? <p className="text-sm text-amber-600">Este período está cerrado. Reabrilo para modificar los pagos.</p> : <form onSubmit={submit} className="grid gap-3 sm:grid-cols-[1fr_1fr_1fr_auto]"><input type="number" min="0" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="Monto" className={inputClass} /><DatePicker value={date} onChange={setDate} /><PaymentSelect value={method} onChange={setMethod} /><button type="submit" aria-label="Agregar pago" className="rounded-xl bg-zinc-900 px-4 py-2 text-sm font-semibold text-white"><Plus size={16} /></button></form>}{payments.length ? <div className="mt-5 space-y-2">{payments.map((payment) => <div key={payment.id} className="flex items-center justify-between border-t border-zinc-100 pt-2 text-sm dark:border-zinc-800"><span>{payment.date} · {payment.method}</span><div className="flex items-center gap-3"><strong className="text-lime-600">$ {payment.amount.toLocaleString("es-AR")}</strong>{!readOnly && <button onClick={() => removePayment(payment.id)} aria-label="Eliminar pago" title="Eliminar pago" className="text-zinc-400 hover:text-red-500"><X size={14} /></button>}</div></div>)}</div> : <p className="mt-4 text-sm text-zinc-400">Todavía no registraste pagos.</p>}</Card>; }
...
export function HoursPanel({ period, setPeriod, payments, setPayments, entries, setEntries, closedPeriods, setClosedPeriods, onSaveEntry, onRemoveEntry, compact = false }) {
  ...
  const printRows = [...inPeriod].sort((a, b) => a.date.localeCompare(b.date)).map((entry) => ({ date: entry.date, hours: duration(entry), extra: entry.extra, holiday: entry.holiday }));
